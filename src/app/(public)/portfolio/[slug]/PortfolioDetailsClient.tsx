...
import { PortfolioDetailsSkeleton } from "@/components/ui/Skeletons";
...
export default function PortfolioDetailsClient({
  ...
  const { data: project, isLoading, isError, refetch } = usePortfolioDetails(slug);
  const queryClient = useQueryClient();

  const handlePrefetchRelated = (rSlug: string) => {
    queryClient.prefetchQuery({
      queryKey: queryKeys.projects.detail(rSlug),
      queryFn: async () => {
  ...
  if (isError || !project) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <ErrorState
          onRetry={refetch}
          message="Failed to load project case study details."
        />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main pb-24 transition-colors duration-250">
      ...
      {/* Related Projects */}
      {initialRelatedProjects.length > 0 && (
        <section className="max-w-[1400px] mx-auto px-6 md:px-12 mt-20 pt-12 border-t border-border-main">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {initialRelatedProjects.map((relProject: any, idx: number) => (
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    ...
                    <div className="pt-4 border-t border-border-main flex items-center justify-between">
                      ...
                      <Link
                        ...
                        onMouseEnter={() => handlePrefetchRelated(relProject.slug)}
